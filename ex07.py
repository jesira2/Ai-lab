...
class NaiveBayesGaussian:
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        jll = self._joint_log_likelihood(
...
def _make_multinomial_spam_data() -> Tuple[
    np.ndarray,
    np.ndarray,
    np.ndarray,
    np.ndarray,
...
if __name__ == "__main__":
    ...
    Xtr_g, ytr_g, Xte_g, yte_g = (
        _make_gaussian_blob_data(seed=7)
    )
    ...
    print(
        f"[GaussianNB] Test accuracy: {acc_g:.3f}"
    )

    print(
        "Forward-looking note: suitable for "
        "continuous features with class-conditional "
        "normality.\n"
    )

    # Multinomial NB demo

    Xtr_m, ytr_m, Xte_m, yte_m, vocab = (
        _make_multinomial_spam_data()
    )

    mnb = NaiveBayesMultinomial(
        alpha=1.0
    ).fit(Xtr_m, ytr_m)
    ...
    print(
        f"[MultinomialNB] Test accuracy: {acc_m:.3f}"
    )
    ...
    if acc_g >= 0.5 and acc_m >= 0.5:
        print(
            "Thus, both Naive Bayes models were "
            "trained, evaluated, and executed successfully."
        )
    else:
        print(
            "Thus, the implementation executed, "
            "but model accuracy is low; consider "
            "revisiting data or smoothing."
        )
